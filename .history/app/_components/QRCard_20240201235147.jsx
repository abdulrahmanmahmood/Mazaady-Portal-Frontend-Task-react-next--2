      ...
      <div className="flex flex-col ">
        <div className="p-2 bg-white  flex flex-row items-center justify-between">
          ...
          <div className="flex flex-row gap-3 lg:gap-5  items-center">
            ...
            <button onClick={toggleVisibility} className="lg:hidden bg-[#FBE7EE] rounded-xl">
              <Image src={arrow} width={24} height={24} alt="arrow" className={`${isRotated ? "rotate-180" : ""}`}/>
            </button>
          </div>
        </div>

        {/* Conditionally render the content based on the 'isHidden' state */}
        {!isHidden && (
        ...
        <div className={` mx-auto w-full p-6 ${isHidden ? 'hidden' : 'block'} bg-gradient-to-r from-red-500 via-red-600 to-orange-500 rounded-2xl`}>
          <div className="p-2 bg-white flex flex-col items-center gap-2 rounded-2xl outline-dashed justify-around">
            <Image
              src={logo}
              width={150}
              height={50}
